...
@router.message(F.text.lower() == 'work')
async def start(message: types.Message, state: FSMContext):
    await message.answer('Вы хотите работать?', reply_markup=make_keyboard(available_work))
    await state.set_state(Choice.work)


@router.message(Choice.work, F.text.in_(available_work))
...
@router.message(Choice.work)
async def work_incorrectly(message: types.Message):
    await message.answer('Неправильно. Попробуйте ещё раз', reply_markup=make_keyboard(available_work))
